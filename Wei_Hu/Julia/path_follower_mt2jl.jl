...
function CFTOC(N, z0, goal, zmin, zmax, umin, umax, TS)
	...
	for j in 1:N
		...
		@constraint(model, z[1,j]+TS*z[3,j]*cos(z[4,j]+u[2,j]) .== z[1, j+1])
		@constraint(model, z[2,j]+TS*z[3,j]*sin(z[4,j]+u[2,j]) .== z[2, j+1])
		@constraint(model, z[3,j]+TS*u[1,j] .== z[3, j+1])
		@constraint(model, z[4,j]+TS*z[3,j]*sin(u[2,j])/1.738 .== z[4, j+1])
	end
...
while norm(z[1:2] - final) > 2
    ...
    zOpti, uOpti, JOpti = CFTOC(N, z0, goal, zMin, zMax, umin, umax, TS);
